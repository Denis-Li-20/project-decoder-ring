  ...
  */
  ...
  function generatePolybiusCipher(encode = true){
    ...
    for (let i = 0; i < alphabet.length; i++){
      if (column == 6) { //number of columns -1
        column = 1;
        row+=1;
      }
      if (alphabet[i] == "("){ //adding key:value for combined keys
        polybiusCipher[alphabet.substring(i,i+5)] = `${column}${row}`;
        column +=1;
        i += 4;
      } else { //adding key:value for single keys
        polybiusCipher[alphabet[i]] = `${column}${row}`;
        column +=1;
      }
    }
    if(encode === true){ //returning (key = letter, value = code)
      return polybiusCipher;
    } else { //returning (key = code, value = letter)
  ...
  function polybius(input, encode = true) {
    ...
    for (let i = 0; i < input.length; i+=encodeFlag){
      ...
      } else { //expensive search for combined letters only (rarely utilized)
